...
export const TaskList: React.FC<TaskListProps> = ({ 
  ...
  // Split filtering functions to reduce cognitive complexity
  const matchesSearchTerm = (task: Task, searchTerm?: string): boolean => {
  ...
  const matchesCategory = (task: Task, category?: string): boolean => {
    if (!category || category === 'all') return true;
    return task.category.name.toLowerCase() === category.toLowerCase();
  };

  const matchesStatus = (task: Task, status?: TaskStatus | TaskStatus[]): boolean => {
  ...
  const matchesPriority = (task: Task, priority?: TaskPriority): boolean => {
  ...
  const matchesBudgetRange = (task: Task, budgetRange?: TaskFilterParams['budgetRange']): boolean => {
    ...
    if (budgetRange.min !== undefined && task.budget < budgetRange.min) return false;
    if (budgetRange.max !== undefined && task.budget > budgetRange.max) return false;
    return true;
  };

  const filterTasks = (tasks: Task[], filters: TaskFilterParams): Task[] => {
    return tasks.filter(task => 
      matchesSearchTerm(task, filters.search) &&
      matchesCategory(task, filters.category) &&
      matchesStatus(task, filters.status) &&
      matchesPriority(task, filters.priority) &&
      matchesOwner(task, filters.ownerId) &&
      matchesAssignee(task, filters.assigneeId) &&
      matchesBudgetRange(task, filters.budgetRange)
    );
  };
