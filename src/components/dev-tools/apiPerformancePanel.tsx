...
import { useRateLimiter } from '@/services/api/services/consolidatedRateLimiter';
...
function getColorForValue(value: number, thresholds: [number, number, number, number]): string {
  const [danger, warning, info] = thresholds;
...
function PerformanceScore({ score }: { score: number }) {
  // Apply a color based on the score
  ...
  return (
    <div className="flex flex-col items-center">
      <div className={`text-2xl font-bold ${colorStyle}`}>{score}</div>
...
function RateLimitingStatus() {
  ...
  useEffect(() => {
    if (!rateLimiter.isLimited) return;
    
    const interval = setInterval(() => {
      setCountdown(rateLimiter.timeUntilReset);
      if (rateLimiter.timeUntilReset <= 0) {
  ...
  if (!rateLimiter.isLimited && rateLimiter.queueLength === 0) {
    return (
      <Card className="bg-green-50 dark:bg-green-950/20">
        ...
        <CardContent>
          <div className="text-sm">
            ...
            {rateLimiter.degradationLevel > 0 && (
              <div className="mt-1 text-amber-600">
                <AlertTriangle className="h-3 w-3 inline mr-1" />
                Degradation Level: {rateLimiter.degradationLevel}
              </div>
            )}
          </div>
        </CardContent>
      ...
      <CardHeader className="pb-2">
        <div className="flex justify-between">
          ...
          <Badge variant="outline" className="text-amber-700 bg-amber-100 dark:bg-amber-950/50">
            {rateLimiter.isLimited ? 'Limited' : 'Queueing'}
          </Badge>
        </div>
      </CardHeader>
      <CardContent>
        <div className="space-y-2">
          ...
          {rateLimiter.queueLength > 0 && (
            <div className="text-sm">
              <RefreshCw className="h-3 w-3 inline mr-1" />
              Queued requests: {rateLimiter.queueLength}
            </div>
          )}
          <div className="text-sm">
            <AlertTriangle className="h-3 w-3 inline mr-1" />
...
export function ApiPerformancePanel() {
  ...
  useEffect(() => {
    ...
    const updateStats = () => {
      ...
      setEndpointStats(performanceMonitor.getAllEndpointStats());
      setSlowEndpoints(performanceMonitor.getSlowestEndpoints(5));
      setUnreliableEndpoints(performanceMonitor.getLeastReliableEndpoints(5));
    };
