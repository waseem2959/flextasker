...
export function useCompleteTask(): UseMutationResult<ApiResponse<Task>, AppError, string | CompleteTaskParams> {
  ...
  return useMutation<ApiResponse<Task>, AppError, string | CompleteTaskParams>({
    mutationFn: (params) => {
      ...
      const data = typeof params === 'string' ? undefined : { rating: params.rating, feedback: params.feedback };
      
      return taskService.completeTask(taskId, data);
    },
    onSuccess: (_, params) => {
...
export { useTask as useTaskDetails };
export { useMyTasks as useUserTasks };
