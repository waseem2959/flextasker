...
// Import our enhanced API features
...
function RealTimeSyncExample() {
  ...
  const { isConnected, connectionState } = socket;
  const [events, setEvents] = useState<Array<{ type: string; data: any; time: string }>>([]);
  const queryClient = useQueryClient();
  
  // Subscribe to real-time events
  useEffect(() => {
    // Listen for task updates
    const taskUpdatedUnsubscribe = socket.on(SocketEventType.TASK_UPDATED, (data) => {
      // Add to our events log
      setEvents(prev => [
        { type: 'TASK_UPDATED', data: data as Record<string, unknown>, time: new Date().toLocaleTimeString() },
    ...
    // Listen for new bids
    const bidReceivedUnsubscribe = socket.on(SocketEventType.BID_RECEIVED, (data) => {
      setEvents(prev => [
        { type: 'BID_RECEIVED', data: data as Record<string, unknown>, time: new Date().toLocaleTimeString() },
        ...prev.slice(0, 9)
      ]);
    });
    
    // Listen for notifications
    const notificationReceivedUnsubscribe = socket.on(SocketEventType.NOTIFICATION_RECEIVED, (data) => {
      setEvents(prev => [
        { type: 'NOTIFICATION', data: data as Record<string, unknown>, time: new Date().toLocaleTimeString() },
        ...prev.slice(0, 9)
      ]);
    });
